...
$(function () {
  ...
  var new_scale_x = d3.scale.linear().domain([min_x, max_x]).range([(window.screen.availWidth/2)-10, 10]);
  var new_scale_y = d3.scale.linear().domain([min_y, max_y]).range([window.screen.availHeight-10, 10]);
  for (var i = 0; i < Object.keys(nnodes).length; i++) {
  ...
  var svg = d3.select("#svg").append("svg")
    .attr("width", window.screen.availWidth/2)
    .attr("height", window.screen.availHeight-20)
    .call(zoom)
    .append('g');
       ...
       if (toggle == 0) {
           ...
           node.style("opacity", function (o) {
             if (d == o)
                ...
                return neighboring(d, o) | neighboring(o, d) ? .5 : 0.1;
           });
