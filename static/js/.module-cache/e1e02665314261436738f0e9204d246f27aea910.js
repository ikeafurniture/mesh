...
$(function () {
  ...
  var new_scale_x = d3.scale.linear().domain([min_x, max_x]).range([(window.innerWidth/2)-20, 20]);

  var new_scale_y = d3.scale.linear().domain([min_y, max_y]).range([window.innerHeight-20, 20]);
  for (var i = 0; i < Object.keys(nnodes).length; i++) {
