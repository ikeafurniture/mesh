...
$(function () {
  ...
  var new_scale_x = d3.scale.linear().domain([min_x, max_x]).range([(window.screen.innerWidth/2)-10, 10]);
  var new_scale_y = d3.scale.linear().domain([min_y, max_y]).range([window.screen.innerHeight-10, 10]);
  for (var i = 0; i < Object.keys(nnodes).length; i++) {
  ...
  var svg = d3.select("#svg").append("svg")
    .attr("width", window.screen.innerWidth/2)
    .attr("height", window.screen.innerHeight)
    .call(zoom)
    .append('g');
