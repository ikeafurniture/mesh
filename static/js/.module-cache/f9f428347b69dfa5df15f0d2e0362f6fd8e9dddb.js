...
var eedges = edge_data;
 ...
 var new_scale_x = d3.scale.linear().domain([min_x,max_x]).range([690,10]);
 var new_scale_y = d3.scale.linear().domain([min_y, max_y]).range([290,10]);
 for(var i = 0; i < Object.keys(nnodes).length; i++){
...
svg = svg.append('g');
      // svg.attr('transform', 'translate(550, -50)');
