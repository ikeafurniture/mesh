...
$(function () {
  ...
  var new_scale_x = d3.scale.linear().domain([min_x, max_x]).range([(window.innerWidth*.4-(window.innerWidth*.025)), (window.innerWidth*.025)]);

  var new_scale_y = d3.scale.linear().domain([min_y, max_y]).range([window.innerHeight-50, 50]);
  for (var i = 0; i < Object.keys(nnodes).length; i++) {
